...
class AirApi {
    ...
    async newAccessToken({ username, password } = {}) {
        if (!username) {
            console.log("Airbnbapi: Can't apply for a token without a username.");
            return null;
        } else if (!password) {
            console.log("Airbnbapi: Can't apply for a token without a password.");
            return null;
        }
        const options = this.buildOptions({
            token: 'public',
        ...
        /*try {
            ...
            if (response && response.access_token) {
                console.log(`Airbnbapi: Successful login for [${username}], auth ID is [${response.access_token}]`);
                return { token: response.access_token };
            } else {
                console.log('Airbnbapi: no response from server when fetching token');
                return null;
            }
        } catch (e) {
            ...
            console.log("Airbnbapi: Couldn't get auth token for " + username);
            console.log(e.error);
    ...
    async login({ email, password } = {}) {
        if (!email) {
            console.log("Airbnbapi: Can't login without an email.");
            return null;
        } else if (!password) {
            console.log("Airbnbapi: Can't apply for a token without a password.");
            return null;
        ...
        try {
            ...
            if (response && response.login) {
                console.error(`Airbnbapi: Successful login for [${email}], auth ID is [${response.login.id}]`);
                return response;
            } else {
